...
class FeederTopology:
    ...
    def __init__(self, dss: py_dss_interface.DSS, source_bus: str, target_kv: float = 12.66, tolerance: float = 0.1):
        ...
        # Dicionário para armazenar os detalhes dos reguladores/transformadores
        self.macro_transformers = {}
    ...
    def _build_full_topology(self):
        ...
        for _ in range(self.dss.transformers.count):
            ...
            # Identifica as fases reais de conexão deste enrolamento
            phases = buses[0].split('.')[0].lower()
            if not phases:
                num_phases = self.dss.cktelement.num_phases
                phases = [str(i+1) for i in range(num_phases)]

            # Se a aresta já existe (ex: banco de transformadores reg1a, reg1b, reg1c),
            # o nx.DiGraph apenas mantém uma única conexão.
            if self._is_target_voltage(b1) and self._is_target_voltage(b2):
                macro_name = name
                if self.dss.cktelement.num_phases == 1 and name[-1] in ['a', 'b', 'c']:
                    macro_name = name[:-1]

                if not self.graph.has_edge(b1, b2):
                    self.graph.add_edge(b1, b2, label=macro_name, type='transformer')
                    self.macro_transformers[macro_name] = []

                self.macro_transformers[macro_name].append({
                    'name': name,
                    'phases': phases,
                    'num_phases': self.dss.cktelement.num_phases
                })
    ...
    def _discover_line_circuits(self) -> Dict[str, List[str]]:
        ...
        Retorna as seções de circuito ordenadas, INCLUINDO os reguladores macro.
        """
        ...
        for i, leaf in enumerate(leaves):
            try:
                ...
                for j in range(len(path_nodes) - 1):
                    ...
                    # AGORA INCLUÍMOS OS TRANSFORMADORES MACRO NA VARREDURA!
                    if edge_data['type'] in ['line', 'transformer']:
    ...
    def get_main_branch(self) -> List[str]:
        if not self.main_circuits: return []
        comprimentos = {}
        for nome, linhas in self.main_circuits.items():
            # Conta o tamanho apenas se for uma linha mapeada (ignorando os macros temporariamente)
            total_l = sum(self.line_data[l]['length'] for l in linhas if l in self.line_data)
    ...
    def print_circuits_info(self):
        ...
        for idx, (nome_circuito, linhas) in enumerate(circuitos_ordenados, start=1):
            ...
            for linha in linhas:
                # O elemento pode ser uma linha ou um regulador macro
                if linha in self.line_data:
